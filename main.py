...
def menu():
    """
This makes the menu load and prompts the user to select an option of their
 preference.
    """
    loop_progress = True
    while loop_progress:
...
def calorie_calculator():
    """
This function is designed to calculate the user's calories based off of
their body's metrics. Function then prompts user to decide if they want to
 lose fat or gain muscle, and it returns the proper amount of calories to eat.
    """
...
def workout_program():
    """
This function is designed to provide workouts for each muscle group that the
user wants to train. If they can't decide there is a random option to generate
 one of the following in the list.
    """
    import random
    workout_type = input((
...
def cardio_routine():
    """
This function provides the user with cardiovascular exercise at their choice.
...
def calorie_tracker():
    """
This function is designed for tracking the user's calories they consume daily.
    """
    ...
    while calories >= 0:
        count += 1
        total_cals += calories
        calories = int(input("Enter the numerical amount of "
                             "calories for the food you consumed: "))
        if count >= 1:
            user_input = input(
                "Do you want to enter another item? (yes/no) ")
            if user_input == "yes":
                calories = int(
                    input("Enter the numerical number of "
                          "calories for the food you consumed: "))
                total_cals += calories
            else:
                total_cals += calories
